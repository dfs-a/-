...
if __name__ == '__main__':
    ...
    #     print('输入成功,稍等片刻')
    #     drivers.get("http://www.cqooc.net/login")
    #     time.sleep(2)
    #     drivers.find_element_by_xpath('//*[@id="loginForm"]/ul/li[1]/input').send_keys(name)
    #     time.sleep(2)
    #     drivers.find_element_by_xpath('//*[@id="loginForm"]/ul/li[2]/input').send_keys(password)
    #     time.sleep(2)
    #     drivers.find_element_by_id('loginBtn').click()
    #     time.sleep(2)
    #     cookies = drivers.get_cookies()
    #     user_id = cookies[0]['value']
    # except:
    #     print('用户登录有误,请重新输入!\n')
    ...
    # window = tk.Tk()
    # window.title('重庆在线课程平台')
    # window.geometry('500x300')
    # var = tk.StringVar()
    # text1 = tk.Entry(window,show=None,font=('Arial',9)).place(x=150, y=100,)
    #
    #
    # def a():
    #     print(text1)
    ...
    # tk.Label(window, text='请输入你的xsid值:', font=('Arial', 10), ).place(x=30, y=100, )
    # b = tk.Button(window, text='确定', font=('Arial', 10), width=8, height=1, command=a).place(x=300, y=100,)
    # # b.pack()
    # # text1.pack()
    # window.mainloop()
    # try:
    #     user_id = input('请输入你的xsid值:')
    # except:
    #     print('输入xsid有误,请重新输入!')
    # platforms = platform(user_id)
    # owner_id,User_name = platforms.User_infomation()
    # #获取报了多少课程
    # lesson_data_list = platforms.Public_Class_Infomation(owner_id)
    # # print(lesson_data_list)
    # while True:
    #     try:
    #         id = input('请选择课程(id序号):')
    ...
    # # platforms.section(courseId)
    # # print(complete_code_list)
    # platforms.start_time(User_name,courseId)
    #
    # sections = platforms.section(courseId).items()
    # # 课程列表与课程总数
    # course_list = platforms.course_number(courseId)
    # # print(course_list)
    # for i in course_list:
    #     section_Id = i['section_Id']
    #     chapter_Id = i['chapter_Id']
    #     # 已完成课程编码列表，与个数
    #     complete_code_list = platforms.complete_course(courseId, User_name)
    #     unfinished = len(course_list) - len(complete_code_list)
    #     print('总课程{}/未完成{}/已完成{}'.format(len(course_list),unfinished,len(complete_code_list)))
    #     if section_Id not in complete_code_list:
    #         print('-----访问成功,等待半分钟----')
    #         platforms.start_time(User_name, courseId)
    #         platforms.add_server(section_Id,User_name,courseId)
    #         time.sleep(30)
    #         platforms.start_time(User_name, courseId)
    #         time.sleep(2)
    #         platforms.request_code(chapter_Id,courseId,owner_id,parentId,section_Id,User_name)
    #         print('-----课程完成------')
    #     else:
    #         print('-----已完成，跳过此课程-----')
    #         time.sleep(5)
